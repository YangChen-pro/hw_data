...
它会把 workflow 里的跳转关系渲染成一张可缩放、可点击的流程图。节点和边都可以点开看详情，若仓库里已有最近一次评估报告，还会自动给节点附加问题徽标。

当前 viewer 的图谱区是按排障场景设计的，不是普通流程图：

- 默认优先展示当前节点及其上下游，不会一上来缩成全图缩略图
- 支持 `当前子图 / 全图 / 异常路径 / 分组聚焦`
- 右下角 minimap 是固定 overlay，只做导航辅助，不参与主图布局
- 点击分组后，右侧会切到分组摘要，而不是普通节点详情
- 点击节点或边后，视图会尽量保证当前焦点可读
